...
### files
files = dir("data/metadonnees", pattern = "M?ta")

### load and merge data --------
surface.strate=data.frame(STRATE=1:3, surface.strate = c(29.61e+9,21.91e+9,17.93e+9))
volu=readexcel(file='data/Volume_N-P.xls',sheet=c('SOGolfe 1982-1998'))
volu=rename(volu,c('ANNÉE'='year','STATION'='station')) # in correspondance with the next file
...
eggs=data.frame()
for(i in files){
    sheets = getSheets(loadWorkbook(paste0("data/metadonnees/",i))) #load sheet names
    ...
    metado=readexcel(file=paste0("data/metadonnees/",i),sheet=nrs)
    data=metado[[1]]
    planc=metado[[2]]
    ...
    ##cleanup
    if(data$year[1]==2007){planc[planc$clé.unique==7 &!is.na(planc$clé.unique),'station']=2.2} #2.2a to 2.2 (there is also a 2.2b, but plancton sample from a)
...
save(eggs,file='Rdata/eggs.Rdata')
 #load('Rdata/eggs.Rdata')
...
savepng(grid.arrange(T.Nm2.plot,T.cdf.plot,ncol=2,bottom='Temperature'),"./img/SST/",'temp',c(18,10))
...
savepng(grid.arrange(T.Nm2.plot,T.cdf.plot,ncol=2,bottom='Temperature'),"./img/SST/",'temp_year',c(18,10))
...
for(i in unique(eggs$year)){
  ...
  T.Nm2.plot=ggplot(eggs[eggs$year==i,],aes(x=temp,y=Nm2))+geom_point() +xlab('') +ylab('Abundance (N/m2)')+ 
    ...
    scale_colour_continuous(low=c('red','yellow'),high='blue')+
    theme(legend.position='none')
  savepng(grid.arrange(T.Nm2.plot,T.cdf.plot,ncol=2,bottom='Temperature'),"./img/SST/Yearly/",paste0('temp_',i),c(18,10))
}
...
# grid
grille = read.table("data/grille_pred.csv", header = TRUE, sep=";")
...
savepng(plot.bubble,"./img/spatial/",'bubble',c(35,20))
...
varios=readexcel(file='data/Varios_mackerel.xlsx',sheet=c(1))
...
for(i in unique(eggs$year)){
    ...
    savepng(print(plot(my.vgm, model=v, plot.numbers=T, pch=16)),"./img/spatial/variograms/",paste0('vario_',i),c(20,12))
...
savepng(grid.arrange(plot.krig.pred+ggtitle('Predicted'),plot.krig.var+ggtitle('Variance'),ncol=2,top=i),
        "./img/spatial/krigging/",paste0('krig_',i),c(28,12))
...
for(i in sort(unique(eggs$year))){
    ...
    savepng(grid.arrange(logi.plot,prob.plot,ncol=1,bottom='Day of the year'),
            "./img/logistic/",paste0('fit_',i),c(20,18))
...
png(filename="img/DOY_spawning_vs_survey.png",units="cm",res=200,width=20,height=12)
...
dist$date <- format(as.Date(dist$day, origin = paste0(dist$year,"-01-01")), format="%m-%d")
spawning.prop$date <- format(as.Date(spawning.prop$peak.day, origin = paste0(spawning.prop$year,"-01-01")), format="%b-%d")

pspaw = ggplot(dist,aes(x=day,y=prob*100))+
    geom_ribbon(aes(ymax=prob*100),ymin=0,fill="grey",colour=NA)+
    geom_vline(data=spawning.prop,aes(xintercept=peak.day))+
    geom_text(data=spawning.prop,aes(y=0,x=peak.day,label=date),vjust=-1,hjust=0)+
    facet_grid(~year)+
    xlab('Spawning day')+ylab('')+
    coord_flip()+
...
savepng(pspaw,"./img/",'spawning_day',c(25,8))
