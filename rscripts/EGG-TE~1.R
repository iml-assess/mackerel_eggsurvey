...
load("C:/Users/SmithAND/Desktop/mackerel_egg_index_2022/rdata/df_egg_index_2022_v4.Rdata")
load("C:/Users/SmithAND/Desktop/mackerel_egg_index_2022/rdata/egg_index_stage15_1979-2022.Rdata")

# quick look at data
egg %>% ggplot(aes(temperature, n_m2)) + geom_point()
df_egg_index %>% dplyr::filter(stage=="n_15") %>% 
  ggplot(aes(temperature, n_m2)) + geom_point()

# get regularly spaced temperature data and calculate geometric mean
df <- egg %>% dplyr::filter(!is.na(temperature),!is.na(n_m2))
library(FSA)
df %<>% mutate(temp = lencat(temperature, w=0.1))
df %<>% mutate(n_m2 = remove_outliers(n_m2)) %>% dplyr::filter(!is.na(n_m2))
df <- df %>% group_by(temp) %>%
    dplyr::summarise(geo_mean_n_m2 = gm_mean(n_m2,na.rm=T), mean_n_m2 = mean(n_m2,na.rm=T)) 
...
m.2 <- x[which.max(y2)]     # peak 
s.2 <- (max(x)-min(x))/4   # rate 
b.2 <- min(y2)              # baseline
a.2 <- (max(y2)-min(y2))     # overall magnitudes of relative y values
...
fit2 <- nls(log(y2) ~ f(x, c(m, s, a, b)),
           data.frame(x, y2),
           start = list(m = m.2, s = s.2, a = a.2, b = b.2))

# Get error stats on model as not available with nls by using MC simulation
newdata = data.frame(x = sort(unique(df$temp)))
df_fit <- as.data.frame(predictNLS(fit,newdata=newdata))
df_fit %>% ggplot(aes(exp(fit),mean))+geom_point()
as.data.frame(predictNLS(fit)) %>% ggplot(aes(exp(fit),mean))+geom_point()
...
abline(v = summary(fit)$parameters["m", 1], add = TRUE, col ="blue", lwd = 2 )
abline(v = summary(fit)$parameters["m", 1]+(summary(fit)$parameters["m", 2]*1.96), add = TRUE, col ="blue", lwd = 1,lty=2 )
abline(v = summary(fit)$parameters["m", 1]-(summary(fit)$parameters["m", 2]*1.96), add = TRUE, col ="blue", lwd = 1,lty=2 )
text(x = 5, y = 3, "Peak temperature = 12.859698 +/- 0.175335", col ="blue")
...
#  Figure
df %>% 
ggplot(aes(x = temp, y = log(geo_mean_n_m2))) + geom_point() +
    stat_function(fun = fmean, col = 'red', size = 1.5) +
    scale_x_continuous(limits = c(0,25)) +
    scale_y_continuous() +
    labs(x = "Temperature", y = "log mean egg density (N*m-2)") +
    theme_minimal(base_size = 14) +
    geom_vline(xintercept  = summary(fit)$parameters["m", 1]) +
    geom_vline(xintercept = summary(fit)$parameters["m", 1]+(summary(fit)$parameters["m", 2]*1.96), linetype = "dashed") +
    geom_vline(xintercept = summary(fit)$parameters["m", 1]-(summary(fit)$parameters["m", 2]*1.96), linetype = "dashed") +
    annotate("text", x = 7, y = 5, label = "Peak = 12.859698 +/- 0.175335")
...
load("C:/Users/SmithAND/Desktop/mackerel_egg_index_2022/rdata/mackerel_bio_lf_2022.Rdata")
...
# get regularly spaced temperature data and calculate geometric mean
df <- mackerel_bio %>% dplyr::filter(!is.na(doy),!is.na(gsi), maturity_stage >3, doy>135)
...
#            start = list(m = m.0, s = s.0, a = a.0, b = b.0))
# 
# fit<-nls(gsi ~ NLS.L4(doy, b, c, d, e), data = df) #

# Get error stats on model as not available with nls by using MC simulation
newdata = data.frame(x = sort(unique(df$doy)))
